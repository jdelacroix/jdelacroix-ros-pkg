...
Lion::Lion() {
	...
	if(motion_capture_system_.compare("optitrack") == 0) {
	  position_subscriber_ = node_handle_.subscribe("/optitrack/data", 1, &Lion::optitrack_callback, this);
	} else if(motion_capture_system_.compare("vicon") == 0) {
	  position_subscriber_ = node_handle_.subscribe("/vicon/data", 1, &Lion::vicon_callback, this);
	} else if(motion_capture_system_.compare("odometry") == 0) {
	  position_subscriber_ = node_handle_.subscribe("khepera3/odometry", 1, &Lion::odometry_callback, this);
	} else {
...
void Lion::optitrack_callback(const optitrack_driver::OptiTrackData &msg) {

    if(msg.id == lion_id_) { // Lion
...
void Lion::vicon_callback(const vicon_driver::ViconData &msg) {
...
void Lion::odometry_callback(const geometry_msgs::Pose &msg) {
...
void Lion::compute_control_signal(void ) {
	...
	// g = [g_x;g_y]
	double g_x = gazelle_pose_.x;
	double g_y = gazelle_pose_.y;
...
double Lion::enforce_bounds(double value, double bound) {
	if(value > bound) {
		return bound;
	} else if(value < -bound) {
		return -bound;
